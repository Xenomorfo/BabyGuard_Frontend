...
class Dashboard extends StatefulWidget {
  ...
  const Dashboard(
      {super.key, required this.user, this.conn});
        ...
        child: Scaffold(
          ...
          body: FutureBuilder(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData && snapshot.data['events']['timestamp'] != 0.1
              && snapshot.data['serial'].toString().length == 10) {
                 return Container(
                      ...
                      child: GridView.count(
                        ...
                        children: <Widget>[
                          makeDashboardItem(
                              readTimestamp(snapshot.data['events']['timestamp'])
                                  .toString(),
                              Icons.timelapse,
                              'Ultimo Evento', Color.fromRGBO(51, 153, 255, 1.0)),
                          if (snapshot.data['events']['seat'] == 1)
                        makeDashboardItem('Presente',
                            Icons.present_to_all, 'Criança',
                            Color.fromRGBO(0, 150, 0, 1.0))
                      else
                        makeDashboardItem('Ausente',
                            Icons.present_to_all, 'Criança',
                      ...
                      if (snapshot.data['events']['belt'] == 1)
                        ...
                        makeDashboardItem('Aberto',
                            Icons.lock, 'Cinto',
                            Color.fromRGBO(222, 215, 25, 1.0)),
                      // Cinto de segurança
                      if (snapshot.data['events']['car'] == 1)
                        makeDashboardItem('Fechada',
                            Icons.car_rental, 'Viatura',
                            Color.fromRGBO(0, 150, 0, 1.0))
                      else
                        makeDashboardItem('Aberta',
                            Icons.car_rental, 'Viatura',
                      ...
                      if (snapshot.data['events']['blue'] == 1)
                        ...
                        makeDashboardItem('Desemparelhado',
                            Icons.bluetooth, 'Bluetooth',
                            Color.fromRGBO(222, 215, 25, 1.0)),
                      // Cinto de segurança
                      if (snapshot.data['events']['temperature'] > 30)
                        makeDashboardItem(
                            snapshot.data['events']['temperature'].toString() +
                                " ºC",
                            Icons.thermostat, 'Temperatura',
                            Color.fromRGBO(250, 0, 0, 1.0))
                      else
                        makeDashboardItem(
                            snapshot.data['events']['temperature'].toString() +
                      ...
                      if (snapshot.data['events']['humidity'] > 70)
                        makeDashboardItem(
                            snapshot.data['events']['humidity'].toString() +
                                " %",
                            Icons.water_drop, 'Humidade',
                            Color.fromRGBO(250, 0, 0, 1.0))
                      else
                        makeDashboardItem(
                            snapshot.data['events']['humidity'].toString() +
